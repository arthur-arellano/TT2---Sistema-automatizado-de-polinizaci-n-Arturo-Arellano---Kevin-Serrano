...
class MotorController:
    ...
    def acercar_vertical(self):
        ...
        logger.info(f"⬇️ Acercando vertical: {DISTANCIA_VERTICAL}mm")
        self.mover_vertical(DISTANCIA_VERTICAL, 1)
    
    def alejar_vertical(self):
        """Alejar de la planta (subir)"""
        logger.info(f"⬆️ Alejando vertical: {self.posicion_vertical_mm}mm")
        if self.posicion_vertical_mm > 0:
            self.mover_vertical(self.posicion_vertical_mm, 0)
    ...
    def polinizar_planta(
        ...
        Secuencia:
        1. Mover horizontalmente a la planta
        2. Esperar estabilización (2s)
        3. Acercar verticalmente
        4. Activar vibración
        5. Alejar verticalmente
        """
        if self.is_active:
        ...
        try:
            ...
            logger.info(f"⏳ Esperando estabilización ({DELAY_ESTABILIZACION}s)")
